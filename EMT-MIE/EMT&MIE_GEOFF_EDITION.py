...
directory = './Input Files for EMT-MIE.py/'

if not preDMat:
    #Amorphous Carbon Optical Constants
    dir_AC = directory + 'AmorphCarbOptConst.dat'
    
    #Water Optical Constants
    dir_WA = directory + 'waterOpticalConstants.dat'

else:
   #Dirty Ice Optical Constants
    dir_DI = directory + 'DirtyIceOptConst.dat' 
    
#AstroSil Optical Constants
dir_AS = directory+ 'AstroSilOptConst.dat'

#Desired Wavelengths
dir_W = directory + 'wav_set.dat'

#Desired Grain Sizes; current table is currently unreadable by my modules so it needs to manually
#be modified; sorry!
dir_G = directory + 'grain_size_grid_suvsil.list'
...
#End short EMT function... yes this is really the extent of it.
...
if 0:
  test=np.logspace(-3.,3.,121)
  for i in range(len(test)):
    print 'sizes match?',Waves['SIZE'][i],test[i]
  test=np.logspace(-3.,5.,1201)
  for i in range(len(test)):
    print 'waves match?',Waves['WAV'][i],test[i]
  exit('just making sure they match')
Waves['SIZE']=np.logspace(-3.,3.,121)
Waves['WAV']=np.logspace(-3.,5.,1201)
#Waves['SIZE']=np.logspace(-3.,3.,20)
#Waves['WAV']=np.logspace(-3.,5.,21)

G_Tab.close()

print 'grid size (wavelengths,grain radii):',len(Waves['WAV']),len(Waves['SIZE'])
...
if not preDMat:
   ...
   for i, ele in enumerate(DirtyIce['N']):
        
        #Utilize EMT with the Predetermined Matrix and Inclusion
        N, K = EMT(ele, DirtyIce['K'][i], AstroSil['N'][i], AstroSil['K'][i])
...
#            print i,radius
    ...
    print 'size of Emiss:',Emiss.shape
