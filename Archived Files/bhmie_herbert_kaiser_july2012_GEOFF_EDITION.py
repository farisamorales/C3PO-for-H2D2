...
# why?  this looks like a fortran dimensioning issue
#    if (nang > 1000):
...
#        return
    ...
    #qback = 4*(abs(s1[2*nang-2])/dx)**2     #Unecessary for this implimentation
    #qback = ((abs(s1[2*nang-2])/dx)**2 )/pii  #old form

	thetas=arccos(amu)
# actually, needs to be longer array to get full 2pi
	thetas=arange(0.0,2*nang-1,1)
	thetas=thetas*dang
	sinthetas=sin(thetas)
	phases=absolute(s1)**2 + absolute(s2)**2
#    phases=phases[1:]
#    print 'lens',len(sinthetas),len(phases)
#    norm = sum(sinthetas*phases*dang)
#    normcheck = sum(sinthetas*dang)
#    print 'norm check',normcheck
#    phases = 2.*phases/norm #Normalize such that int P dOmega = 4Pi
	norm = sum(sinthetas*phases*dang)/sum(sinthetas*dang)
	phases = phases/norm 

    #return s1,s2,qext,qsca,qback,gsca, \     #Only need (qext-qsca) here
    #      thetas,phases
	return (qext-qsca)