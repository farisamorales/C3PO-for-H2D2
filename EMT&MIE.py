...
if not preDMat:
	...
	AmorphCarb = {'WAV':mList[0], 'N':mList[1], 'K':mList[2], 'NAME':'Amorphous Carbon'}
	...
	Water = {'WAV':mList[0], 'N':mList[1], 'K':mList[2], 'NAME':'Water Ice'}
	...
	DirtyIce = {'WAV':mList[0], 'N':mList[1], 'K':mList[2], 'NAME':'Dirty Ice'}
...
mList[3].reverse()
mList[4].reverse()
AstroSil = {'WAV':mList[0], 'N':mList[3], 'K':mList[4], 'NAME':'Astro Silicates'}
...
IMPOptConst = {'WAV':Waves['WAV'], 'N':[], 'K':[], 'NAME':'Inclusion Matrix Particles'}
...
#--------------------------------------------------
	...
	def flot(*flargs):
		...
		plt.grid(True)
		for Exti in flargs:
			plt.plot(Exti['WAV'], Exti['K'], label = Exti['NAME'])
		plt.legend()
		plt.show()
		...
		for Refracti in flargs:
			plt.plot(Refracti['WAV'], Refracti['N'], label = Refracti['NAME'])
		plt.legend(loc = 2)
		plt.show()
...
#Delete old dictionaries to free up memory for the Emiss array	
	...
	for radius in Waves['SIZE']:
		...
		for i, lamda in enumerate(Waves['WAV']):
			...
			#print(radius, lamda, N, K, i, len(Emiss), len(Emiss[radius]))
